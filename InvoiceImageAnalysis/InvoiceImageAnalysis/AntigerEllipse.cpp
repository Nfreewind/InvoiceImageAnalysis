...
void CEllipseDetectorYaed::ClusterEllipses(vector<Ellipse>& ellipses)
{
	...
	// Group near ellipses using disjoint-set
	float max_distance2 = _fMaxCenterDistance2;
	vector<int> labels;
	int n_labels = cv::partition(ellipses, labels, [max_distance2](const Ellipse& lhs, const Ellipse& rhs)
	{
		float dis2 = ((lhs._xc - rhs._xc)*(lhs._xc - rhs._xc) + (lhs._yc - rhs._yc)*(lhs._yc - rhs._yc));
		if (dis2 < max_distance2)
			return true;
		else
			return false;
	});

	vector<Ellipse> groupedEllipses(n_labels);
	vector<int> flags(n_labels, int(0));
	int i;
	for (i = 0; i < iNumOfEllipses; i++)
	{
		if (flags[labels[i]] == 0)
		{
			groupedEllipses[labels[i]] = ellipses[i];
			flags[labels[i]] = 1;
		}
		else
		{
			if (ellipses[i]._score > groupedEllipses[labels[i]]._score)
			{
				groupedEllipses[labels[i]] = ellipses[i];
			}
		}
	}
	groupedEllipses.swap(ellipses);

	//float th_Da = 0.25f;//0.1f
	//float th_Db = 0.25f;//0.1f
	//float th_Dr = 0.25f;//0.1f

	//float th_Dc_ratio = 0.25f;//0.1f
